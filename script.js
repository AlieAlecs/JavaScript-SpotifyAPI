...
function displayResults(tracks, tableBodyId) {
  ...
  tracks.forEach(track => {
      ...
      row.innerHTML = `
          ...
          <td>
              <button class="go-to-track-button" onclick="redirectToTrack('${track.external_urls.spotify}')">Go to Track</button>
...
function cycleRecommendations() {
  searchSpotify(); // Refresh the recommendations table by re-fetching recommendations
}

function scrollToRecommendation(index) {
  const recommendationsBody = document.getElementById('recommendations-body');
  const yOffset = recommendationsBody.children[index].offsetTop - recommendationsBody.offsetTop;
  recommendationsBody.scrollTop = yOffset;
}

function toggleLike(button) {
  button.classList.toggle('liked');
  const likeButton = button;
  const trackRow = likeButton.parentNode.parentNode;
  let trackUrl = trackRow.querySelector('.go-to-track-button').getAttribute('onclick');
  
  // Inject 'embed' in the path to construct the embedded link
  trackUrl = trackUrl.replace('open.spotify.com/', 'open.spotify.com/embed/');
}